...
def validate_per_step_records(records: list[dict[str, Any]]) -> ValidationReport:
    ...
    * ``epistemic + pragmatic ≈ efe`` for every (agent, t).
    ...
    report.add(
        "posterior_stochastic",
        bad_posterior == 0,
        f"{bad_posterior}/{len(records)} bad posterior vectors",
    )
    report.add("prior_stochastic", bad_prior == 0, f"{bad_prior}/{len(records)} bad prior vectors")
    report.add("efe_finite", bad_efe == 0, f"{bad_efe}/{len(records)} non-finite EFE vectors")
    report.add(
        "q_pi_stochastic", bad_qpi == 0, f"{bad_qpi}/{len(records)} bad policy-posterior vectors"
    )
    report.add(
        "p_u_stochastic", bad_pu == 0, f"{bad_pu}/{len(records)} bad action-marginal vectors"
    )
    report.add(
        "efe_decomposition_consistent",
        bad_decomp == 0,
...
def validate_generative_model(agent: Any) -> ValidationReport:
    ...
    * Every quantity is finite (no NaN / inf).
    ...
    if A is not None:
        A = np.asarray(A)
        report.add(
            "A_shape", A.shape == (n_obs, n_states), f"expected {(n_obs, n_states)}, got {A.shape}"
        )
        report.add("A_finite", bool(np.isfinite(A).all()))
        col_sums = A.sum(axis=0)
        report.add(
            "A_column_stochastic",
            bool(np.allclose(col_sums, 1.0, atol=1e-8)),
    ...
    if B is not None:
        ...
        report.add(
            "B_column_stochastic_per_action",
            all_ok,
            f"worst (min, max) = {worst}" if not all_ok else None,
        )
    if C is not None:
        C = np.asarray(C)
        report.add("C_shape", C.shape == (n_obs,), f"expected {(n_obs,)}, got {C.shape}")
        report.add(
            "C_normalised", bool(np.isclose(C.sum(), 1.0, atol=1e-8)), f"sum = {C.sum():.6f}"
        )
    if D is not None:
        D = np.asarray(D)
        report.add("D_shape", D.shape == (n_states,), f"expected {(n_states,)}, got {D.shape}")
        report.add(
            "D_normalised", bool(np.isclose(D.sum(), 1.0, atol=1e-8)), f"sum = {D.sum():.6f}"
        )
    return report
