"""Animated trajectory rendering."""

from __future__ import annotations

from pathlib import Path

import matplotlib

matplotlib.use("Agg")
import matplotlib.pyplot as plt
import pandas as pd
from matplotlib.animation import FuncAnimation, PillowWriter

from blockference.viz._common import extract_agent_positions, infer_grid_dimension

__all__ = ["animate_trajectory"]


def animate_trajectory(
    df: pd.DataFrame,
    out_path: str | Path,
    *,
    fps: int = 4,
    title: str = "Agent trajectories",
    grid_dim: int | None = None,
) -> Path:
    """Render a validated trajectory as a GIF or MP4."""

    path = Path(out_path)
    if isinstance(fps, bool) or not isinstance(fps, int) or fps < 1:
        raise ValueError("fps must be a positive integer")
    if path.suffix.lower() not in {".gif", ".mp4"}:
        raise ValueError("unsupported animation extension; use .gif or .mp4")
    path.parent.mkdir(parents=True, exist_ok=True)
    positions = extract_agent_positions(df)
    inferred = infer_grid_dimension(df)
    n = inferred if grid_dim is None else grid_dim
    if isinstance(n, bool) or not isinstance(n, int) or n < 2 or n != inferred:
        raise ValueError(f"grid_dim must equal the trajectory dimension {inferred}")
    n_frames = len(next(iter(positions.values())))
    fig, ax = plt.subplots(figsize=(6, 6))
    try:
        ax.set_xlim(-0.5, n - 0.5)
        ax.set_ylim(n - 0.5, -0.5)
        ax.set_xticks(range(n))
        ax.set_yticks(range(n))
        ax.grid(True, linestyle="--", alpha=0.4)
        ax.set_aspect("equal")
        ax.set_title(title)
        cmap = plt.get_cmap("tab10")
        lines, dots, labels = {}, {}, {}
        for index, agent_id in enumerate(positions):
            color = cmap(index % 10)
            (line,) = ax.plot([], [], "-", color=color, alpha=0.6, label=f"agent {agent_id}")
            (dot,) = ax.plot([], [], "o", color=color, markersize=10)
            lines[agent_id] = line
            dots[agent_id] = dot
            labels[agent_id] = ax.text(0, 0, "", color=color, fontsize=8)
        ax.legend(loc="upper right")
        step_text = ax.text(0.02, 0.97, "", transform=ax.transAxes, va="top", fontsize=10)

        def init():
            for line in lines.values():
                line.set_data([], [])
            for dot in dots.values():
                dot.set_data([], [])
            step_text.set_text("")
            return list(lines.values()) + list(dots.values()) + [step_text]

        def update(frame: int):
            for agent_id, trajectory in positions.items():
                up_to = trajectory[: frame + 1]
                ys, xs = zip(*up_to, strict=True)
                lines[agent_id].set_data(xs, ys)
                dots[agent_id].set_data([xs[-1]], [ys[-1]])
                labels[agent_id].set_position((xs[-1] + 0.05, ys[-1] - 0.1))
                labels[agent_id].set_text(str(agent_id))
            step_text.set_text(f"step {frame}")
            return list(lines.values()) + list(dots.values()) + [step_text]

        animation = FuncAnimation(
            fig, update, frames=n_frames, init_func=init, blit=False, interval=1000 / fps
        )
        if path.suffix.lower() == ".gif":
            animation.save(path, writer=PillowWriter(fps=fps))
        else:
            from matplotlib.animation import FFMpegWriter

            animation.save(path, writer=FFMpegWriter(fps=fps))
    except (ImportError, RuntimeError) as exc:
        raise RuntimeError("MP4 output requires ffmpeg; use a .gif extension") from exc
    finally:
        plt.close(fig)
    return path
